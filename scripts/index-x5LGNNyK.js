import { Ct as e, Dt as t, Et as n, S as r, Tt as i, _t as a, ft as o, ht as s, ut as c, vt as l, wt as u, yt as d } from "./react-w7ldCLEn.js";
...
var m = /* @__PURE__ */ new WeakMap(), h = () => {}, g = void 0, _ = Object, v = (e) => e === g, y = (e) => typeof e == "function", b = (e, t) => ({
	...
	initFocus: N,
	initReconnect: P
}, ne = !c.useId, L = !w || E, re = (e) => D() ? window.requestAnimationFrame(e) : setTimeout(e, 1), R = L ? i : d, z = typeof navigator < "u" && navigator.connection, ie = !L && z && (["slow-2g", "2g"].includes(z.effectiveType) || z.saveData), B = /* @__PURE__ */ new WeakMap(), V = (e) => _.prototype.toString.call(e), H = (e, t) => e === `[object ${t}]`, ae = 0, U = (e) => {
...
var W = (e, t) => {
	...
	let { value: n } = e, r = u(Z), i = y(n), a = l(() => i ? n(r) : n, [
		i,
		r,
		n
	]), o = l(() => i ? a : X(r, a), [
		...
		r,
		a
	]), c = a && a.provider, d = s(g);
	c && !d.current && (d.current = ue(c(o.cache || q), a));
	let f = d.current;
	return f && (o.cache = f[0], o.mutate = f[1]), R(() => {
		if (f) return f[2] && f[2](), f[3];
	}, []), t(Z.Provider, b(e, { value: o }));
};
//#endregion
...
var Q = w && window.__SWR_DEVTOOLS_USE__, fe = Q ? window.__SWR_DEVTOOLS_USE__ : [], pe = () => {
	Q && (window.__SWR_DEVTOOLS_REACT__ = c);
}, me = (e) => y(e[1]) ? [
	e[0],
	e[1],
	e[2] || {}
...
}), be = { dedupe: !0 }, xe = Promise.resolve(g), Se = () => h, Ce = (t, n, r) => {
	let { cache: i, compare: o, suspense: c, fallbackData: u, revalidateOnMount: d, revalidateIfStale: f, refreshInterval: p, refreshWhenHidden: h, refreshWhenOffline: _, keepPreviousData: x, strictServerPrefetchWarning: S } = r, [C, w, T, E] = m.get(i), [D, O] = oe(t), k = s(!1), A = s(!1), j = s(D), M = s(n), N = s(r), P = () => N.current, F = () => P().isVisible() && P().isOnline(), [I, z, ie, B] = te(i, D), V = s({}).current, H = v(u) ? v(r.fallback) ? g : r.fallback[D] : u, ae = (e, t) => {
		for (let n in V) {
			let r = n;
			if (r === "data") {
				if (!o(e[r], t[r]) && (!v(e[r]) || !o(Y, t[r]))) return !1;
		...
		let e = I(), t = B(), r = (e) => {
			let t = b(e);
			return delete t._k, !(!D || !n || P().isPaused()) && (U && !v(d) ? d : v(v(H) ? t.data : H) || f) ? {
				isValidating: !0,
				isLoading: !0,
				...t
			} : t;
		}, i = r(e), a = e === t ? i : r(t), o = i;
		return [() => {
			let e = r(I());
			return ae(e, o) ? (o.data = e.data, o.isLoading = e.isLoading, o.isValidating = e.isValidating, o.error = e.error, o) : (o = e, e);
		}, () => a];
	}, [i, D]), W = (0, ve.useSyncExternalStore)(e((e) => ie(D, (t, n) => {
		ae(n, t) || e();
	}), [i, D]), se[0], se[1]), ue = C[D] && C[D].length > 0, G = W.data, K = v(G) ? H && ee(H) ? ye(H) : H : G, q = W.error, J = s(K), Y = x ? v(G) ? v(J.current) ? K : J.current : G : K, X = D && v(K), Z = s(null);
	!L && (0, ve.useSyncExternalStore)(Se, () => (Z.current = !1, Z), () => (Z.current = !0, Z));
	let de = Z.current;
	S && de && !c && X && console.warn(`Missing pre-initiated data for serialized key "${D}" during server-side rendering. Data fetching should be initiated on the server and provided to SWR via fallback data. You can set "strictServerPrefetchWarning: false" to disable this warning.`);
	let Q = !D || !n || P().isPaused() || ue && !v(q) ? !1 : U && !v(d) ? d : c ? v(K) ? !1 : f : v(K) || f, fe = U && Q, pe = v(W.isValidating) ? fe : W.isValidating, me = v(W.isLoading) ? fe : W.isLoading, $ = e(async (e) => {
		let t = M.current;
		if (!D || !t || A.current || P().isPaused()) return !1;
		let n, i, a = !0, s = e || {}, c = !T[D] || !s.dedupe, l = () => ne ? !A.current && D === j.current && k.current : D === j.current, u = {
			isValidating: !1,
			isLoading: !1
		}, d = () => {
			z(u);
		...
		try {
			if (c && (z(p), r.loadingTimeout && v(I().data) && setTimeout(() => {
				a && l() && P().onLoadingSlow(D, r);
			}, r.loadingTimeout), T[D] = [t(O), ce()]), [n, i] = T[D], n = await n, c && setTimeout(f, r.dedupingInterval), !T[D] || T[D][1] !== i) return c && l() && P().onDiscarded(D), !1;
			u.error = g;
			let e = w[D];
			if (!v(e) && (i <= e[0] || i <= e[1] || e[1] === 0)) return d(), c && l() && P().onDiscarded(D), !1;
			let s = I().data;
			u.data = o(s, n) ? s : n, c && l() && P().onSuccess(n, D, r);
		} catch (e) {
			f();
		...
		return a = !1, d(), !0;
	}, [D, i]), he = e((...e) => le(i, j.current, ...e), []);
	if (R(() => {
		M.current = n, N.current = r, v(G) || (J.current = G);
	}), R(() => {
		if (!D) return;
		let e = $.bind(g, be), t = 0;
		...
		X && (M.current = n, N.current = r, A.current = !1);
		let e = E[D];
		if (ye(!v(e) && X ? he(e) : xe), !v(q) && X) throw q;
		let t = X ? $(be) : xe;
		!v(Y) && X && (t.status = "fulfilled", t.value = !0), ye(t);
	}
	return {
		mutate: he,
		get data() {
